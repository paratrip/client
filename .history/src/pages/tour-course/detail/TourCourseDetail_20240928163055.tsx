...
const TourCourseDetail: React.FC = () => {
  ...
  const { result } = useKakaoMapSearch(data?.paraglidingName);
  ...
  const [mapCenter] = useState<MapCenter>(previous => {
    return { ...previous, lat: String(result?.lat), lng: String(result?.lng) };
  });
